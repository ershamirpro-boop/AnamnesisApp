...
# רענון שקט כל 5 דק' (אם החבילה זמינה)
try:
...
COMPLAINTS: Dict[str, Dict[str, Any]] = {
    ...
    "דפיקות לב": {
        ...
        "labs": [L("TSH, T4","תירוטוקסיקוזיס"), L("אלקטרוליטים כולל Mg","הפרעת קצב"), L("CBC","אנמיה")],
    ...
    "בצקות ברגליים": {
        ...
        "scores": [S("Framingham HF","קריטריונים ל-HF"), S("NYHA","דרגת תפקוד"), S("Wells DVT","סיכון ל-DVT","≥2 - US","<2 + D-dimer שלילי")],
        "notes": []
    },
    "סינקופה": {
    ...
    "COPD – החמרה": {
        ...
        "physical_exam": [{"label":"סטורציה ו-RR"},{"label":"האזנה - צפצופים/קראקלס"}],
        "labs": [L("ABG/VBG","Hypercapnia/Acidosis","מצוקה נשימתית"), L("טרופונין","נזק לבבי","קלינית מתאימה")],
        "imaging": [IMG("צילום חזה","לחיפוש סיבוך/זיהום")],
        "scores": []
    },
    "חשד לדלקת ריאות": {
        "questions": ["חום/צמרמורות/כיח","כאב פלאוריטי/קוצר נשימה","גורמי סיכון/Aspiration"],
        "physical_exam": [{"label":"האזנה לריאות (קראקלס)"}],
    ...
    "סחרחורת": {
        "questions": ["תנוחתי/אי יציבות? התקפי/מתמשך?","שמיעה/טינטון/נוירו נוספים?","מדללי דם/לחצי דם לא מאוזנים"],
    ...
    "כאב ראש": {
        "questions": ["thunderclap? החמרה חדשה?","פוטופוביה/בחילה/חסך נוירולוגי","צד/מעיר משינה/מגיב לאנלגטיקה","דלקת כלי דם/הריון/מדללים?"],
    ...
    "פרכוס": {
        ...
        "physical_exam": [{"label":"בדיקה נוירולוגית ממוקדת"}],
        "labs": [
            L("לקטט/CK","פוסט־איקטלי"),
            L("גלוקוז","היפוגליקמיה"),
            L("VBG/ABG","חמצת/אוורור"),
            L("אלקטרוליטים","דיסאלקטרולמיה"),
            L("שתן לטוקסיקולוגיה","חשד לשימוש")
        ],
    ...
    "כאב אפיגסטרי/דיספפסיה": {
        ...
        "imaging": [
            IMG("US/CT","לפי קליניקה"),
            IMG("ECG","אם יש חשד מרכיב לבבי")
        ]
    },
    ...
    "כאב מותני - חשד לאבן": {
        "questions": ["כאב התקפי מקרין למפשעה","בחילות/המטוריה","אבנים בעבר","חום/הקאות"],
    ...
    # --- א.א.ג ועיניים ---
    "דימום אפי ספונטני": {
        "questions": ["שימוש במדללי דם","חד/דו צדדי, טראומה/חיטוט","יתר ל\"ד?"],
    ...
    "פריחה בעור": {
        "questions": ["משך/החמרה, גרד/כאב","עקיצה/נשיכה/חיית מחמד","חזרה מחו\"ל","חום/תרופות/חומר חדש"],
    ...
    "כאב מפרק חד": {
        ...
        "labs": [L("חומצה אורית, CBC, CRP","דלקת"), L("שאיבת נוזל","קריסטלים/תרבית")],
        "imaging": [IMG("צילום מפרק","לשבר/ארתרוזיס")]
    },
    "התייבשות": {
        "questions": ["שלשול/הקאות/חום/צריכת אלכוהול","תרופות משתנות/ACEi/NSAIDs"],
        "physical_exam": [{"label":"התייבשות/טורגור"},{"label":"מדדים וחום רקטאלי"}],
        "labs": [L("BMP","אוראה/קרטינין/Na/K/אלבומין"), L("Hb","המוקונסנטרציה")]
    },
    "לחץ דם נמוך/שוק": {
...
def render_scores(scores: Any) -> None:
    ...
    for s in scores:
        if isinstance(s, dict):
            name = s.get("name",""); about=s.get("about",""); ri=s.get("rule_in",""); ro=s.get("rule_out",""); ref=s.get("ref","")
